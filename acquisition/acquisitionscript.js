...
//create variables for axes, no tick marks, position horizontally and vertically
	var xAxis = d3.svg.axis()
		.scale(x)
		.ticks(0)//shows no tick marks, ideal since we only need to show Low/High
		.orient("bottom");
...
//This is the data file itself, contains all x/y coordinates by year
	d3.csv("acquire.csv", function(error, data) {

//set axes length
		x.domain([0, 30000]).nice();//nice rounds the scale values, not needed here but useful for scales in general
...
//Controls transitions and sets transition duration when Play button is pushed.	
...
//Drives all transitions when slider is moved.
